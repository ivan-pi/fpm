submodule (fpm_source_parsing) fpm_source_parsing_impl

use fpm_error, only: file_parse_error, fatal_error, file_not_found_error
use fpm_strings, only: string_t, string_cat, len_trim, split, lower, str_ends_with, fnv_1a, is_fortran_name
use fpm_model, only: FPM_UNIT_UNKNOWN, FPM_UNIT_PROGRAM, FPM_UNIT_MODULE, &
...
!>```
!>
module function parse_f_source(f_filename,error) result(f_source)
...
!>
module function parse_c_source(c_filename,error) result(c_source)
...
end submodule fpm_source_parsing_impl