submodule (fpm_manifest_package) fpm_manifest_package_impl

    use fpm_manifest_build, only: new_build_config
    use fpm_manifest_dependency, only : new_dependencies
    use fpm_manifest_profile, only : new_profiles, get_default_profiles
    use fpm_manifest_example, only : new_example
    use fpm_manifest_executable, only : new_executable
    use fpm_manifest_fortran, only : new_fortran_config
    use fpm_manifest_library, only : new_library
    use fpm_manifest_install, only: new_install_config
    use fpm_manifest_test, only : new_test
    use fpm_manifest_preprocess, only : new_preprocessors
    use fpm_manifest_metapackages, only: new_meta_config
    use fpm_filesystem, only : exists, getline, join_path
    use fpm_error, only :fatal_error, syntax_error, bad_name_error
    use fpm_toml, only : toml_array, toml_key, toml_stat, get_value, len, &
